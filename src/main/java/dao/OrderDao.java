...
public class OrderDao extends GenericDao<Order> {
    ...
    public List<Order> findAllContainingProduct(Product product) {
        openSession();
        Query query = session.createQuery("Select o from Order join  fetch o.products p where p = :product").setParameter("product", product);
        List<Order> orders = query.getResultList();
        session.close();
        return orders;
    }
    ...
    public List<Order> findByUser(User user) {
        openSession();
        Query query = session.createQuery("Select o from Order join  fetch o.user u where u = :user").setParameter("user", user);
        List<Order> orders = query.getResultList();
        session.close();
        return orders;
    }
